...
const ChatBody = (props) => {
    ...
    const fetchChatMessage = async () => {
        const currentChatId = CommonUtil.getActiveChatId(props.match);
        if (currentChatId) {
            const url =
            ...
            props.onMessagesChange(chatMessages);
        }
    };

    useEffect(() => {
        fetchChatMessage();
    }, [CommonUtil.getActiveChatId(props.match)]);
        ...
        if (inputMessage) {
            props.socket.send(
                JSON.stringify({
                    ...
                    roomId: CommonUtil.getActiveChatId(props.match),
                })
            );
        }
        setInputMessage("");
            ...
            <div className="py-2 px-4 border-bottom d-none d-lg-block">
                <div className="d-flex align-items-center py-1">
                    ...
                    <div className="flex-grow-1 pl-3">
                        <strong>{props.activeChat?.name}</strong>
                    </div>
                </div>
            </div>
            <div className="position-relative">
                ...
                >
                    ...
                    {props.messages?.results?.map((message, index) => (
